...
- **`createDiagnostic` helper** — answers "why is my harness silent?" in one printout. Exports from `@harnesskit/provider-fetch`. Counts events on the bus, detects whether `fetch` carries the `HARNESSKIT_PATCHED` symbol marker, surfaces common misconfigurations (no install, host doesn't match a provider, error events). `report()` for structured access; `format()` for a console-friendly summary. Showcase: `examples/src/showcase-diagnostic.ts` — three setups (forgot install / unregistered proxy host / correct) with the diagnostic next to each.
- **`rateLimit` policy** — sliding-window TPM/RPM cap. Observes `usage` and `turn.start` events; denies `tool.call.requested` when the window is saturated. Configurable window length (default 60s), injectable clock for tests. Sits alongside `tokenBudget` (totals) and `costBudget` (dollars). Showcase: `examples/src/showcase-rate-limit.ts`.
